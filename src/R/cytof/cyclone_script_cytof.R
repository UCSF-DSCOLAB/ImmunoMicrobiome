...
### NOTES:
# Assumes that:
#      the FCS data doesn't have and need compensation
...
#      all FCS files have the same set of channels and the corresponding markers (This is a common sense, but still spelling it out here)
#      
# 
# Scaffold assumptions:
# The clustering is performed on all files together.
...
suppressMessages({
  ...
  #library(ggalluvial)
  #library(hexbin) # density visualization in ggplot2
  library(gridExtra) # plot grid of graphs
  #library(grid) # plot grid of graphs
  #library(forcats)
  library(pals) #colors
  #library(ggrepel) #for label repel on ggplot2
  library(cowplot)
  library(pheatmap)
  library(ggpubr)
  library(ggExtra)
  ...
  ##  heatmap packages
  #library(superheat) #For heatmaps... should I make a heatmap
  #library(dendsort) #dendogram
  #library(ComplexHeatmap) #heatmap
  #library(dendextend) #dendogram 
  
  # Statistical analysis
  #library(siggenes) # better maintained package for SAMR analysis
  #library(stats)
  
  library(optparse)
  library(configr)
  library(igraph)
  # Fetching the data directly from cell engine
  #library(httr)
  #source("api_cellengine.R")
})
...
run_flowsom <- function(trans_exp_submarkers, clust_params) {
  ...
  print_message("FlowSOM clustering begins")
  fSOM <- FlowSOM(my_flowframe,
                  ...
                  xdim = clust_params$xdim, ydim = clust_params$ydim,
  ...
  return(list("clusters" = clusters, "clust_obj" = fSOM))
}
...
if( rerun_from_checkpoint != -1 ) CHECKPOINT <- min( rerun_from_checkpoint, CHECKPOINT )
...
if( CHECKPOINT > 0 ) {
  ...
  for( my_checkpoint in 1:CHECKPOINT ) {
    ...
    if(! file.exists(checkpoint_file) ) {
      ...
      print_message(paste("The", checkpoint_file, "is missing. Delete all checkpoint_*.RData files following the missing file and rerun the script. Exiting."))
      quit(save = "no", status = 1)
    }
